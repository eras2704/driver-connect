...
import { ProfilePhotoInput } from "@/components/gallery/profile-photo-input";

export function DriverForm({ id, initial, services, mode = "admin", vehicleCoverUrl, driverPortraitUrl }: { mode?: "admin" | "driver"; id?: string; initial?: DriverInput; services: { id: string; name: string }[]; vehicleCoverUrl?: string; driverPortraitUrl?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const upload = driverPhoto || vehicleEnabled && vehiclePhoto ? new FormData() : null;
      if (upload) {
        upload.set("profile", JSON.stringify(body));
        if (vehicleEnabled && vehiclePhoto) {
          upload.set("vehiclePhoto", vehiclePhoto);
          upload.set("consent", text("vehiclePhotoConsent"));
        }
        if (driverPhoto) {
          upload.set("driverPhoto", driverPhoto);
          upload.set("driverConsent", text("driverPhotoConsent"));
        }
      }
  ...
  return <form onSubmit={submit} className="driver-form">
    <section className="admin-card"><span className="eyebrow">01 / PERFIL</span><h2>Datos del conductor</h2><div className="form-grid">
      ...
      <input type="hidden" name="photoUrl" value={initial?.photoUrl || ""} /><ProfilePhotoInput kind="driver" currentUrl={driverPortraitUrl || initial?.photoUrl} disabled={pending} error={errors.driverPhoto} onChange={setDriverPhoto} mode={mode} />
    ...
    <div className="form-footer"><Link href={mode === "driver" ? "/panel" : "/admin/conductores"} className="button button-secondary">Volver</Link><button className="button button-primary" type="submit" disabled={pending}>{pending ? driverPhoto || vehiclePhoto ? "Guardando perfil y fotos…" : "Guardando…" : id ? "Guardar cambios" : "Crear conductor"}</button></div>
  </form>;
}
