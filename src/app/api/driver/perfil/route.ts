...
import { readProfileRequest, withProfilePhotos, addVehiclePhoto, replaceDriverPortrait, discardProfilePhoto } from "@/lib/profile-photo";
...
export async function PATCH(request: Request) {
  return handleApi(async () => {
    ...
    const { body, files } = await readProfileRequest(request, actor.userId);
    ...
    const previousPortrait = await withProfilePhotos(files, input.vehicle, photos => db().$transaction(async tx => {
      ...
      await addVehiclePhoto(tx, actor.driverId, photos.vehicle);
      ...
      return replaceDriverPortrait(tx, actor.driverId, photos.driver);
    }));
    await discardProfilePhoto(previousPortrait);
    return json({ ok: true });
  });
}
