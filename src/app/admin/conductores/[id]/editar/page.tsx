...
export default async function EditDriverPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ guardado?: string }> }) {
  ...
  return <><div className="admin-heading"><div><span className="eyebrow">EDITAR PERFIL</span><h1>{driver.name}</h1><p>/conductor/{driver.slug} · {driver.active ? "Publicado" : "Borrador"}</p></div>{driver.active && <Link className="button button-secondary" href={`/conductor/${driver.slug}`} target="_blank" rel="noopener noreferrer">Ver perfil público ↗</Link>}</div>{(await searchParams).guardado === "1" && <p className="form-success" role="status">El perfil se guardó correctamente.</p>}<DriverForm key={driver.updatedAt.toISOString()} id={id} initial={initial} services={services} /></>;
}
