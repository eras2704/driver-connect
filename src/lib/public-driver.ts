...
export async function publicDriver(slug: string) {
  ...
  const { user, portraitStorageKey, ...publicFields } = driver;
  return { ...publicFields, photoUrl: portraitUrl(portraitStorageKey) || driver.photoUrl, bookingEnabled: Boolean(user?.active && driver.services.length), languages: Array.isArray(driver.languages) ? driver.languages.filter((value): value is string => typeof value === "string") : [] };
}
