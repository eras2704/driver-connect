...
export async function publicDriver(slug: string) {
  ...
  const { user, ...publicFields } = driver;
  return { ...publicFields, bookingEnabled: Boolean(user?.active && driver.services.length), languages: Array.isArray(driver.languages) ? driver.languages.filter((value): value is string => typeof value === "string") : [] };
}
