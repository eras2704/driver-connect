...
El script comprueba la aplicación y el acceso desde el proxy, conserva el contenido de `/opt/ccpd/server/Caddyfile`, valida la configuración completa y crea una copia de respaldo privada antes de añadir el bloque NFC. Después recarga `server-caddy-1` mediante Caddy. El proxy actual tiene `admin off`: el script verifica que su versión y proceso admitan `SIGUSR1`, envía esa señal y espera una confirmación de recarga en los registros. Conserva `admin off` y no reinicia el contenedor. Si la recarga falla, intenta restaurar y recargar la configuración anterior; informa si esa recuperación también falla. No añade bloques duplicados al repetirlo. Si una versión anterior mostró «Archivo restaurado, pero no se pudo recargar», sigue [los pasos para actualizar únicamente el script](resolver-puerto-80.md#si-apareció-archivo-restaurado-pero-no-se-pudo-recargar).
